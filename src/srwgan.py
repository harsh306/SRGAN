...
class SRGAN:
	...
	def inference_adversarial_loss_wasserstein(real_output, fake_output): 
		alpha = 1e-3
		g_loss = tf.reduce_mean(tf.scalar_mul(-1, fake_output))
		d_loss = tf.reduce_mean(tf.subtract(fake_output, real_output))
		return (g_loss*alpha, d_loss*alpha)

        content_loss = inference_content_loss(x, imitation)
        generator_loss, discriminator_loss = (
            inference_adversarial_loss_wasserstein(true_output, fake_output))
